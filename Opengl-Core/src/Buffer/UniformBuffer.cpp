...
namespace ogl {
	...
	UniformBuffer::UniformBuffer(const std::string &variable) :
	...
	void UniformBuffer::update(const unsigned int &offset, const unsigned int &size, void *data) const {
		this->bind();
		this->fastUpdate(offset, size, data);
		this->unbind();
	}
	void UniformBuffer::fastUpdate(const unsigned int &offset, const unsigned int &size, void *data) const {
